//-----------------------------------------------------------------------
// <copyright file="FlowSubFlowImpl.cs" company="Akka.NET Project">
...
{
    /// <summary>
    /// A type-preserving decorator over a <see cref="Flow{TIn,TOut,TMat}"/>-rooted
    ...
    /// chaining, and (b) narrows the merge methods to the concrete <see cref="Flow{TIn,TOut,TMat}"/>.
    ///
    /// <para>
    /// The merge downcast is sound: the wrapped substream's merge already produces a
    /// <see cref="Flow{TIn,TOut,TMat}"/> at runtime (the merge-back applies <c>_self.Via(...)</c> on the
    /// original flow); only the static type was widened to <see cref="IFlow{TOut,TMat}"/>.
    /// </para>
    /// </summary>
    ...
    public class FlowSubFlowImpl<TIn, TOut, TMat> : FlowSubFlow<TIn, TOut, TMat>
    {
        private readonly SubFlow<TOut, TMat, Sink<TIn, TMat>> _inner;

        /// <summary>
        /// Creates a decorator over the given <see cref="Flow{TIn,TOut,TMat}"/>-rooted substream.
        /// </summary>
        /// <param name="inner">The substream to wrap.</param>
        public FlowSubFlowImpl(SubFlow<TOut, TMat, Sink<TIn, TMat>> inner) => _inner = inner;

        /// <inheritdoc/>
        public override IFlow<T2, TMat> Via<T2, TMat2>(IGraph<FlowShape<TOut, T2>, TMat2> flow) =>
            new FlowSubFlowImpl<TIn, T2, TMat>((SubFlow<T2, TMat, Sink<TIn, TMat>>)_inner.Via(flow));
        ...
        public override Flow<TIn, TOut, TMat> MergeSubstreamsWithParallelism(int parallelism) =>
            (Flow<TIn, TOut, TMat>)_inner.MergeSubstreamsWithParallelism(parallelism);
    }
}
