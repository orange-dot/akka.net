...
| Status | PR / Branch | Component | Type | Change | Migration |
|--------|-------------|-----------|------|--------|-----------|
| Planned | `codex-5381-subflow-shape-prototype` | `Akka.Streams` | API | `Source.GroupBy` / `SplitWhen` / `SplitAfter` now return `SourceSubFlow<TOut,TMat>`, and `Flow.GroupBy` / `SplitWhen` / `SplitAfter` now return `FlowSubFlow<TIn,TOut,TMat>`, preserving the concrete builder type through representative sub-flow operators and `MergeSubstreams()`. | Source code that stores the result as `SubFlow<...>` remains valid. Recompile binaries that call these members, and prefer the new typed sub-flow return values when continuing with the fluent stream builder API. |
